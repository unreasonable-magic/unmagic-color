...
RSpec.describe(Unmagic::Color::RGB::Gradient::Linear) do
  describe ".color_class" do
    it "returns RGB class" do
      expect(Unmagic::Color::RGB::Gradient::Linear.color_class).to(eq(Unmagic::Color::RGB))
    end
  end

  describe ".build" do
    it "builds gradient from color strings" do
      gradient = Unmagic::Color::RGB::Gradient::Linear.build(["#FF0000", "#0000FF"])
    ...
    it "builds gradient from color objects" do
      ...
      gradient = Unmagic::Color::RGB::Gradient::Linear.build([red, blue])
      expect(gradient.stops.length).to(eq(2))
    ...
    it "auto-balances positions for three colors" do
      gradient = Unmagic::Color::RGB::Gradient::Linear.build(["#FF0000", "#00FF00", "#0000FF"])
      expect(gradient.stops[0].position).to(eq(0.0))
    ...
    it "accepts explicit positions" do
      gradient = Unmagic::Color::RGB::Gradient::Linear.build([["#FF0000", 0.0], ["#00FF00", 0.3], ["#0000FF", 1.0]])
      expect(gradient.stops[0].position).to(eq(0.0))
    ...
    it "accepts mixed positioned and non-positioned colors" do
      gradient = Unmagic::Color::RGB::Gradient::Linear.build(["#FF0000", ["#00FF00", 0.3], "#0000FF"])
      expect(gradient.stops[0].position).to(eq(0.0))
      expect(gradient.stops[1].position).to(eq(0.3))
      expect(gradient.stops[2].position).to(eq(1.0))
    end
    ...
    it "converts colors to RGB color space" do
      gradient = Unmagic::Color::RGB::Gradient::Linear.build(["hsl(0, 100%, 50%)", "hsl(240, 100%, 50%)"])
  ...
  describe "#initialize" do
    it "accepts stops array" do
      ...
      gradient = Unmagic::Color::RGB::Gradient::Linear.new(stops)
    ...
    it "raises error if fewer than 2 stops" do
      ...
      expect do
        Unmagic::Color::RGB::Gradient::Linear.new(stops)
  ...
  describe "#rasterize" do
    ...
    it "generates bitmap with specified width" do
      gradient = Unmagic::Color::RGB::Gradient::Linear.build(["#FF0000", "#0000FF"], direction: direction)
